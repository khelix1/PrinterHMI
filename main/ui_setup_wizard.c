...
static void set_status(const char *text) { if (s_status) lv_label_set_text(s_status, text ? text : ""); }
...
static void wifi_selected_cb(lv_event_t *event)
{
    ...
    set_status(status);
    ui_popup_add_action_at(s_step, UI_POPUP_ACTION_CONFIRM, LV_SYMBOL_RIGHT " NEXT", 512, 480, 260, 44, wifi_password_open_cb, NULL, NULL);
}
...
static void wifi_scan_ready(lv_timer_t *timer)
{
    ...
    for (size_t i = 0; i < s_wifi_count; ++i) {
        char row[56];
        snprintf(row, sizeof(row), "%s  (%d dBm)", s_wifi[i].ssid, (int)s_wifi[i].rssi);
...
static void wifi_scan_open(void)
{
    ...
    if (!s_step) return;
    ui_popup_add_action_at(s_step, UI_POPUP_ACTION_CANCEL, LV_SYMBOL_LEFT " BACK", 28, 480, 260, 44, back_cb, NULL, NULL);
    s_wifi_scan_done = false;
...
static void wifi_password_open(void)
{
    ...
    s_password = ui_popup_add_textarea(s_step, 720, 48, LV_ALIGN_TOP_MID, 0, 140, true, true, 63, "Wi-Fi password", "", NULL);
    ...
    ui_popup_add_action_at(s_step, UI_POPUP_ACTION_CANCEL, LV_SYMBOL_LEFT " BACK", 28, 480, 260, 44, back_cb, NULL, NULL);
    ui_popup_add_action_at(s_step, UI_POPUP_ACTION_CONFIRM, LV_SYMBOL_OK " CONNECT", 512, 480, 260, 44, wifi_save_cb, NULL, NULL);
}
...
static void printer_test_poll(lv_timer_t *timer)
{
    ...
    s_name = ui_popup_add_textarea(s_step, 720, 44, LV_ALIGN_TOP_MID, 0, 156, true, false, 31, "Printer name", s_printer_identity[0] ? s_printer_identity : "Printer 1", NULL);
    ...
    ui_popup_add_action_at(s_step, UI_POPUP_ACTION_CONFIRM, LV_SYMBOL_SAVE " SAVE PRINTER", 512, 480, 260, 44, printer_save_cb, NULL, NULL);
}
...
static void printer_open(void)
{
    ...
    if (!netif || esp_netif_get_ip_info(netif, &ip) != ESP_OK || ip.ip.addr == 0) {
        ...
        if (s_step) ui_popup_add_action_at(s_step, UI_POPUP_ACTION_CANCEL, LV_SYMBOL_LEFT " BACK", 28, 300, 260, 44, back_cb, NULL, NULL);
        return;
    }
...
static void camera_test_poll(lv_timer_t *timer)
{
    ...
    set_status(verified);
    ui_popup_add_action_at(s_step, UI_POPUP_ACTION_CONFIRM, LV_SYMBOL_SAVE " USE CAMERA", 512, 480, 260, 44, camera_save_cb, NULL, NULL);
}
...
static void finish_cb(lv_event_t *event) { (void)event; (void)onboarding_controller_mark_complete(); ui_setup_wizard_close(); }
static void later_cb(lv_event_t *event) { (void)event; ui_setup_wizard_close(); }

static void card(lv_obj_t *parent, const char *title, const char *detail, int x, int y, lv_event_cb_t cb)
{
    lv_obj_t *box = lv_obj_create(parent); lv_obj_set_size(box, 398, 92); lv_obj_set_pos(box, x, y); lv_obj_clear_flag(box, LV_OBJ_FLAG_SCROLLABLE); ui_apply_surface_role(box, UI_SURFACE_SECTION);
    lv_obj_t *heading = lv_label_create(box); lv_label_set_text(heading, title); ui_apply_text_body_large(heading); ui_apply_label_bright(heading); lv_obj_set_pos(heading, 16, 12);
    lv_obj_t *body = lv_label_create(box); lv_label_set_text(body, detail); ui_apply_text_caption(body); ui_apply_label_dim(body); lv_obj_set_width(body, 250); lv_obj_set_pos(body, 16, 44);
    ui_popup_add_action_at(box, UI_POPUP_ACTION_SECONDARY, "OPEN", 286, 22, 96, 48, cb, NULL, NULL);
}
...
static void center_wifi_cb(lv_event_t *event) { (void)event; close_center_for_step(); wifi_scan_open(); }
static void center_printer_cb(lv_event_t *event) { (void)event; close_center_for_step(); printer_open(); }
static void center_camera_cb(lv_event_t *event) { (void)event; close_center_for_step(); camera_open(); }
...
static void show_center(void)
{
    ...
    ui_popup_add_status_label(s_center, "Discovery-first setup. Each step stays in its own guided popup and returns here.", 28, 68, 824);
    card(s_center, "1  CONNECT WI-FI", s_wifi_done ? "Verified and connected." : "Scan, select a network, enter password, and verify.", 28, 116, center_wifi_cb);
    card(s_center, "2  ADD PRINTER", s_printer_done ? "Moonraker printer saved." : "Discover, select, verify, name, and save Moonraker.", 454, 116, center_printer_cb);
    card(s_center, "3  ADD CAMERA", s_camera_done ? "Live camera saved." : "Discover this printer’s cameras, test one, and save.", 28, 220, center_camera_cb);
    card(s_center, "4  READY TO OPERATE", "Finish now, or run Setup Center again later from Settings.", 454, 220, finish_cb);
    ui_popup_add_progress_detail(s_center, "These are dedicated onboarding screens; normal Settings stays unchanged.", 28, 350, 824);
    ui_popup_add_standard_footer_divider(s_center);
    ui_popup_add_action_at(s_center, UI_POPUP_ACTION_CANCEL, "SET UP LATER", 28, 464, 260, 44, later_cb, NULL, NULL);
    ui_popup_add_action_at(s_center, UI_POPUP_ACTION_CONFIRM, LV_SYMBOL_OK " FINISH SETUP", 592, 464, 260, 44, finish_cb, NULL, NULL);
}
