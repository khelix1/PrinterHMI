...
header_path = Path("main/moonraker_config_controller.h")
source_path = Path("main/moonraker_config_controller.c")
...
for marker in required_markers:
    if marker not in old_header:
        raise RuntimeError(
            f"missing controller header marker: {marker}")

for marker in [
    '#define DEFAULT_MOONRAKER_HOST',
    'nvs_set_str(',
    '"moon_host"',
    'bool moonraker_config_load(void)',
]:
    if marker not in old_source:
        raise RuntimeError(
            f"missing controller source marker: {marker}")
...
new_source = r'''#include "moonraker_config_controller.h"
...
print("Installed multi-printer profile foundation.")
print("Capacity: 4 profiles")
print("Existing endpoint migrates automatically to Printer 1")
print("Existing host/port consumers remain unchanged")
print("Legacy keys remain mirrored for rollback compatibility")
