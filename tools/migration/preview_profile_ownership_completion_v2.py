...
reset_pattern = re.compile(
    r"(void ui_printer_v32_preview_reset\(void\)\s*\{"
    r".*?    s_preview_cache_revision = 0;\n)",
    flags=re.S,
)

printer, reset_count = reset_pattern.subn(
    r"\1    s_preview_cache_profile_index = -1;\n",
    printer,
    count=1,
)
...
if reset_count != 1:
    raise RuntimeError(
        "expected one Printer preview reset identity, "
        f"found {reset_count}"
    )
